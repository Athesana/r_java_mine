...
public class B_Aggregate {
	 ...
	 * 		- max()			: 최대 요소를 리턴한다.
	 * 		- min()			: 최소 요소를 리턴한다.
	 * 		- findFirst()	: 첫 번째 요소를 리턴한다. (cf. findany() 무조건 첫 번째 요소가 아닐 수 있음)
	 * 
	 * 커스텀 집계
	...
	public void method1() {
		...
		System.out.println("2의 배수 중에 가장 첫번째 요소 : " + findFirst.getAsInt());
...
//		double avg = values.stream().mapToInt(value -> value.intValue()).average().getAsDouble();
		
		// 빈 값을 처리하는 방법 1 (값의 존재 여부를 확인한다.)
		OptionalDouble average = values.stream().mapToInt(value -> value.intValue()).average();
		
//		System.out.println(average); // OptionalDouble.empty
//		System.out.println(average.isPresent()); // false | 위에 values.add 주석 풀면 true
		...
		double avg = values.stream().mapToInt(value -> value.intValue()).average().orElse(0.0);
		
		System.out.println("평균 : " + avg);
		
		// 빈 값을 처리하는 방법 3 (집계 값을 처리하는 Consumer 등록)
		values.stream().mapToInt(value -> value.intValue()).average().ifPresent(d -> System.out.println("평균 : " + d));
...
//		int[] values = {};
		...
		Arrays.stream(values).reduce((left, right) -> {return left * right;})
		.ifPresent(value -> System.out.println("요소들의 곱은 얼마인가요? " + value));
		
		int result = Arrays.stream(values).reduce(2, (left, right) -> left * right);
