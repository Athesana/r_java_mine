...
public class A_Create {
	 ...
	 *   - 2) 배열로부터 스크림 얻기
	 *   	: Arrays.stream(배열) 메소드로부터 얻어오거나 각 스트림(Stream, IntStream, ...)에 존재하는
...
//		list.stream().forEach(System.out::println);  // 메소드 참조 : 추후 공부해 볼 것
...
//		Stream<String> stream = Arrays.stream(names);
	...
	// 3) 숫자 범위로 스트림 얻기
	public void method3() {
		// 첫 번째 매개 값 ~ 두번 째 매개 값 이전까지( 1 ~9 ) 까지의 값을 요소로 갖는 스트림 객체를 생성한다.
...
//		int sum = stream.sum();
		...
		// 첫 번째 매개 값 ~ 두 번째 매개 값까지 값을 요소루 갖는 스트림 객체를 생성한다.
