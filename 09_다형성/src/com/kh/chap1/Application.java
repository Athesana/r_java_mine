...
/*
 * 다형성
 * 	- 부모 타입으로부터 파생된 여러가지 자식 객체를 부모 클래스의 참조변수(레퍼런스)로 다룰 수 있는 기술
 *  - 즉, 부모 클래스의 참조변수로 다양한 자식 객체를 가질 수 있다.
 * 
 * UpCasting
 *  - 자식 타입을 부모 타입으로 형변환 가능하다.
 *  - 형변환 연산자 생략 가능(자동으로 형변환)
 *  - 자식 타입의 멤버(필드, 메소드)에는 접근 불가
 * 
 * DownCating
 *  - 부모 타입을 자식 타입으로 형변환 가능하다.
 *  - 형변환 연산자 생략 불가능(명적 형변환)
 *  - ((자식타입)부모참조변수).자식메소드();
 * 
 */
public class Application {
	...
	public static void main(String[] args) {
		...
		System.out.println(product.information()); // null, null, null, 0
		
		// 2. 자식 타입의 레퍼런스로 자식 객체를 다루는 경우
		Desktop desktop = new Desktop();
		
		// desktop 참조 변수로 Product, Desktop의 멤버에 접근 가능
		System.out.println(desktop.isAllInOne());   // false	
		System.out.println(desktop.information());  // null, null, null, 0, false
		
		// 3. 부모 타입의 레퍼런스로 자식 객체를 다루는 경우(다형성 적용) -> 업 캐스팅
		Product product2 = /* (Product : 형변환 연산자) */ new Desktop(); 
		
		//product2 참조변수로 Product의 멤버만 접근 가능
		//하지만 Desktop의 멤버에 접근하고 싶으면 형변환을 해줘야 한다. (강제(명시적) 형변환)
		System.out.println(((Desktop)product2).isAllInOne());  // false -> 다운 캐스팅
		System.out.println(product2.information()); // null, null, null, 0, false (자식에서 재정의한대로 나온다)
		...
		// 코드 길이가 감소, 유연한 코딩이 가능
		Product[] arr3 = new Product[4];  // 부모 타입의 객체를 담을 수 있는 배열
		arr3[0] = new Desktop(); // 배열의 각각의 요소에는 Product 타입의 객체를 담을 수 있고, Product를 상속하는 애들은 자식 객체들의 참조 값
		arr3[1] = new Desktop();
		...
		Product product4 = new SmartPhone("삼성", "S-1", "갤럭시 S2", 2000000, "KT");
		Product product5 = new Tv("삼성", "T-1", "스마트TV", 20000000, 60);
//		->부모 타입으로 자식 객체를 참조하고 인포메이션을 호출했다. 각 자식별로 오버라이딩 된 information이 나온다.
		
		/*
		 * 다형성에 오버라이딩의 개념을 접목하게 되면 내가 실제 참조하고 있는 클래스의 메소드를 찾아가서 실행한다.
		 * 동적 바인딩 : 프로그램이 실행되기 전에 컴파일 되면서 모든 메소드는 정적 바인딩 된느데
