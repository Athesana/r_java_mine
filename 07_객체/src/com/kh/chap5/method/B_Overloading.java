...
public class B_Overloading {
	...
	/*
	 * 메소드 오버로딩
	 *  - 한 클래스(type) 내에 같은 이름의 메소드를 여러 개 정의하는 것을 메소드 오버로딩이라고 한다.
	 *  - 메소드 오버로딩은 매개 변수의 **자료형 개수와 순서**가 다르게 작성되어야 하는 원칙이 있다.
	 * 	- 단, <매개 변수명, 접근 제한자, 반환형>은 메소드 오버로딩에 영향을 주지 않는다.
	 */
	...
	/*
	 * 에러가 발생
	 * 	- 매개변수 이름과 상관없이 자료형의 개수와 순서가 같아서 에러가 발생
...
//	}
	 ...
	 *  - 메소드 호출하는 시점에는 반환 값은 중요하지 않는다. 
	 *  - 메소드 호출하는 시점에는 매개 변수가 동일하기 때문에 에러가 발생
...
//	}
	...
	/*
	 * 에러가 발생
	 *  - 접근 제한자가 다르다고 오버로딩이 적용되지는 않는다.
	 *  - 즉, 접근 제한자와 상관 없이 매개 변수의 자료형의 개수와 순서가 다르게 작성되어야 한다.
	 */
