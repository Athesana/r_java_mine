...
public class A_Array {
	...
	public void method2() {
		...
		// 배열 선언
		int[] iArray1;  // 메모리(stack)에 정수형 배열의 주소를 보관할 공간을 할당된다.(참조 변수)
		int iArray2[];  // 메모리(stack)에 정수형 배열의 주소를 보관할 공간을 할당된다.(참조 변수)
		double[] dArray1; // 메모리(stack)에 실수형 배열의 주소를 보관할 공간을 할당된다.(참조 변수)
		double dArray2[]; // 메모리(stack)에 실수형 배열의 주소를 보관할 공간을 할당된다.(참조 변수)
		
		// 배열 할당
//		iArray1 = new int[]; // 배열의 크기를 지정하지 않으면 에러 발생
		iArray1 = new int[5]; // 메모리(heap)에 5개의 정수형 데이터를 저장할 수 있는 배열 생성 -> 배열 시작 주소를 참조 변수에 전달
		iArray2 = new int[10]; // 메모리(heap)에 10개의 정수형 데이터를 저장할 수 있는 배열 생성 -> 배열 시작 주소를 참조 변수에 전달
		dArray1 = new double[5];  // 메모리(heap)에 5개의 실수형 데이터를 저장할 수 있는 배열 생성 -> 배열 시작 주소를 참조 변수에 전달
		dArray2 = new double[10];  // 메모리(heap)에 10개의 실수형 데이터를 저장할 수 있는 배열 생성 -> 배열 시작 주소를 참조 변수에 전달
		...
		// 배열을 생성하고 초기화해주지 않아도 문제없이 출력된다.
		// 배열은 따로 초기화하지 않으면, JVM이 지정한 기본 값으로 배열이 초기화된다.
...
//		 hashCode() 메소드 호출 : 주소값을 10진수 값으로 반환(자바 객체에서 해시 코드는 객체를 식별하는 하나의 정수값을 말한다. -> 중복체크 용도)
		
		// 배열에서 한 번 지정한 크기는 변경이 불가능
		// 따라서 크기를 변경하려면 다시 배열 크기를 지정해서 새로 생성해야 한다.
		iArray = new int[6];
		
		
		// 기존에 참조하고 있던 연결이 끊기고 새로운 배열을 참고하게 된다.
		// 기존에 참조하고 있던 배열은 더 이상 참조하는 변수가 없기 때문에 가비지 컬렉터의 대상이 된다.
		// (특정 시점에 가비지 컬렉터에 의해서 정리된다.)
		System.out.println(iArray.length);
		System.out.println(iArray.hashCode());
	
		// 참조 타입 변수는 힙 영역의 배열(객체)을 참조하지 않는다는 뜻으로 null(널) 값을 가질 수 있다.
		...
		iArray = null;  // -> 더 이상 아무것도 참조하지 않는(사용되지 않는) 상태로 참조 연결을 끊어준다.
		
		
		// 개발자가 null값을 가지고 있는 참조 변수를 사용하면 NullPointerException이 발생한다.
		System.out.println(iArray.length);
		System.out.println(iArray.hashCode());
	...
	public void method4() {
		...
		//{} 사용하려면 반드시 선언과 동시에 초기화 해야 한다. 나눠서 두 번 작성하는거 안됨(아래처럼)
//		iArray2 = new int[]{6, 7, 8, 9, 10};
		...
		//0번 인덱스부터 마지막까지 반복으로 사용자에게 문자열을 입력받아 해당 인덱스에 사용자가 입력한 값 넣기
